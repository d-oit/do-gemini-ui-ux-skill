    ...
    - generic [ref=e132]:
      ...
      - generic [ref=e136]:
        - generic [ref=e138]:
          ...
          - generic [ref=e140]: "0.2712"
        - generic [ref=e144]:
          - generic [ref=e145]: LAYER_1
          - generic [ref=e146]: "0.3477"
        - generic [ref=e150]:
          - generic [ref=e151]: LAYER_2
          - generic [ref=e152]: "0.5179"
        - generic [ref=e156]:
          - generic [ref=e157]: LAYER_3
          - generic [ref=e158]: "0.0285"
        - generic [ref=e162]:
          - generic [ref=e163]: LAYER_4
          - generic [ref=e164]: "0.4776"
        - generic [ref=e168]:
          - generic [ref=e169]: LAYER_5
          - generic [ref=e170]: "0.3591"
```
...
```ts
  ...
  21 |   const visualizer = page.locator('.lg\\:col-span-7').first();
  22 |   const hud = visualizer.locator('.z-20').first();
  23 |   const hudBox = await hud.boundingBox();
  24 |   console.log('HUD Box:', hudBox);
  25 |   
  26 |   const neurons = visualizer.locator('button.z-10');
  27 |   const count = await neurons.count();
  28 |   console.log(`Found ${count} neurons`);
  29 |   for (let i = 0; i < count; i++) {
  30 |     const neuron = neurons.nth(i);
  31 |     const neuronBox = await neuron.boundingBox();
  32 |     
  33 |     // Check label visibility
  34 |     const label = neuron.locator('span');
  35 |     await expect(label).toBeVisible();
  36 |     const labelBox = await label.boundingBox();
  37 |     
  38 |     if (neuronBox && hudBox) {
  39 |       const overlaps = !(
  40 |         neuronBox.x + neuronBox.width < hudBox.x ||
  41 |         neuronBox.x > hudBox.x + hudBox.width ||
  42 |         neuronBox.y + neuronBox.height < hudBox.y ||
  43 |         neuronBox.y > hudBox.y + hudBox.height
  44 |       );
  45 |       console.log(`Neuron ${i} overlaps HUD: ${overlaps}`);
> 46 |       expect(overlaps).toBe(false);
     |                        ^ Error: expect(received).toBe(expected) // Object.is equality
  47 |     }
  48 |     
  49 |     if (labelBox && hudBox) {
  50 |       const overlaps = !(
  51 |         labelBox.x + labelBox.width < hudBox.x ||
  52 |         labelBox.x > hudBox.x + hudBox.width ||
  53 |         labelBox.y + labelBox.height < hudBox.y ||
  54 |         labelBox.y > hudBox.y + hudBox.height
  55 |       );
  56 |       console.log(`Label ${i} overlaps HUD: ${overlaps}`);
  57 |       expect(overlaps).toBe(false);
  58 |     }
  59 |   }
  60 | });
  61 | 
